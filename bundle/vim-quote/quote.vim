...
function! quote#user_input(...)
    if empty(a:000)
        ...
        \ s:mark_pre == "\'" ? "\'" :
        \ s:mark_pre == "\"" ? "\"" :
        \ !empty(s:mark_pre) ? s:mark_pre :
        \ ''
        let s:mark_pos = input('posfix( '.s:predict_pos.' ): ')
        " [add feature] {if no input from mark_pos, define it by the mark_pre}
        if empty(s:mark_pos)
            let s:mark_pos = s:predict_pos
        endif
